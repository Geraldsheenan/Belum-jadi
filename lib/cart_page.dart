...
class _CartPageState extends State<CartPage> {
  ...
  int calculateTotalPrice() {
    ...
    for (var item in widget.cartItems) {
      int price = int.tryParse(item['price'].toString()) ??
          0; // Convert price to int, fallback to 0 if invalid
  ...
  void addProductToCart(Map<String, dynamic> product) {
    setState(() {
      ...
      int index =
          widget.cartItems.indexWhere((item) => item['id'] == product['id']);
  ...
  Widget build(BuildContext context) {
        final cartItems = [
      {'name': 'Produk 1', 'quantity': 2, 'price': 10000},
      {'name': 'Produk 2', 'quantity': 1, 'price': 20000},
    ];
    return Scaffold( 
      backgroundColor: const Color.fromARGB(255, 234, 215, 244),
      appBar: AppBar(
        backgroundColor: const Color.fromARGB(255, 244, 184, 247),
        ...
        leading: IconButton(
          ...
          onPressed: () {
            Navigator.pushReplacement(
                ...
                MaterialPageRoute(
                    builder: (context) =>
                        DashboardPage())); // Return to previous page (Dashboard)
          },
        ),
      ),
      body: widget.cartItems.isEmpty
          ? Center(
              child: Text('Keranjang Anda kosong!',
                  style: TextStyle(color: const Color.fromARGB(255, 229, 112, 243))))
          : Column(
              children: [
                Expanded(
